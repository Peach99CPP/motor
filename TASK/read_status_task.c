...
int Get_Height(void)
{
    //TODO:在进行从左到右和从右到左的扫描时，需注意对模式进行赋值
    ...
    return 11; //都不是的情况下，为避免卡死舵控，执行展开机械臂的动作组
}

/**********************************************************************
  * @Name    Init_Height_Inf
  * @declaration : 初始化高度计算机制，用于在扫描阶梯平台确定角度
  * @param   mode: [输入/出]  从低的开始还是从高的开始
  * @retval   : 无
  * @author  peach99CPP
***********************************************************************/
void Init_Height_Inf(ScanDir_t mode)
{
    Height_Mode = mode;
...
void MV_HW_Scan(int dir, int enable_imu)
{
    ...
    if (dir == 1)
    {
        Init_Height_Inf(Medium_Head); //初始化高度计算机制
    dir1_Start_Symbol:
