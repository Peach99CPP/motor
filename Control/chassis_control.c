...
static int count_line_status, encodermove_status;
int edge_status[3] = {0};
...
void move_by_encoder(int  direct, int val)
{
    ...
    if(direct == 1)
    {
        ...
        if(val < 0)//向左
        {
            ...
            {
                if( (TIME_ISR_CNT - time > 50) && ABS((val - (encoder_sum / 1000)) < ENCODE_THRESHOLD))goto EXIT_FUNC;//超时处理，避免卡死
        ...
        else
        {
            ...
            {
                if( (TIME_ISR_CNT - time > 50) && ABS(val - (encoder_sum / 1000) < ENCODE_THRESHOLD))goto EXIT_FUNC;
                bias = ABS(val - (encoder_sum / 1000));
    ...
    else if(direct == 2)
    {
        ...
        {
            if( (TIME_ISR_CNT - time > 50) && ABS((val - (encoder_sum / 1000) < ENCODE_THRESHOLD)))
                goto EXIT_FUNC;
            bias = fabs(ABS(val) - (encoder_sum / 1000));
            variation = bias * ENCODER_FACTOR;
            variation = variation > MAX_SPEED ? MAX_SPEED : variation;
