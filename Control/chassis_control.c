    ...
    while (1) //姝诲惊鐜�?
    {
        ...
        else if (dir == 2)
        {
            ...
            if (lines < 0)
            {
                ...
                turn_angle(1, 180); //鍏堣浆寮埌180搴︼紝鐒跺悗鍐嶈繘琛屽墠杩涳紝鍥犱负鍙湁姝ｅ墠鏂规湁寰抗鐗�
                lines *= -1;
                osDelay(1000);
            }
...
EXIT_TASK:
    ...
    if (if_need_zero)          //鍒ゆ柇鏄惁闇€瑕佽浆鍥炲師鏉ョ殑瑙掑害
        turn_angle(1, 180);    //杞洖鍘熸潵鐨勮搴�?
    while (!get_turn_status()) //楂橀鑾峰彇杞集鐨勭姸鎬�
        osDelay(1);            //纭繚姝ゆ椂杞集瀹屾垚锛屾墠杩涘叆涓嬩竴闃舵鐨勮鍔�
