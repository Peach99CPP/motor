...
#define EDGE_VAL 7      //边缘数线状态下的循迹读回来的值
    ...
    if((uint8_t)( track_dma[pos][1] + track_dma[pos][2] ) == track_dma[pos][3]) //和校验
    {
        ...
        case 1:
            ...
            if(y_bar.num  >= NUM_THRESHOLD || (edge_status[0] && y_bar.num >= EDGE_THRESHOLD && my_abs(y_bar.data.feedback) >= EDGE_VAL))
            {
                y_bar.line_flag  = 1;//此时到线上
            }
            if(edge_status[0] && y_bar.num >= EDGE_THRESHOLD && my_abs(y_bar.data.feedback) >= EDGE_VAL ) //边缘数线的情况下，特殊处理
        ...
        case 2:
            ...
            if(edge_status[0] && x_leftbar.num >= EDGE_THRESHOLD && ABS(x_leftbar.data.feedback) >= EDGE_VAL)
                x_leftbar.data.feedback = 0;
            if(x_leftbar.num >= NUM_THRESHOLD || (edge_status[0] && x_leftbar.num >= EDGE_THRESHOLD && ABS(x_leftbar.data.feedback) >= EDGE_VAL))
            {
        ...
        case 3:
            ...
            if(edge_status[0] && x_rightbar.num >= EDGE_THRESHOLD && ABS(x_rightbar.data.feedback) >= EDGE_VAL )
                x_rightbar.data.feedback = 0;
            if( x_rightbar.num >= NUM_THRESHOLD || (edge_status[0] && x_rightbar.num >= EDGE_THRESHOLD && ABS(x_rightbar.data.feedback) >= EDGE_VAL ))
            {
